...
func TestRecordBinaryEncoding(t *testing.T) {
	...
	fr, err := ReadFrom(bytes.NewReader([]byte{
		...
		0x03, 0x13, 0x14, 0x15, 0x16, // pkt_observation_t[0]: u8 + u16
		0x00, 0x00, 0x00, 0x00, 0x00, // pkt_observation_t[1]: u8 + u16
		0x00, 0x00, 0x00, 0x00, 0x00, // pkt_observation_t[2]: u8 + u16
		0x00, 0x00, 0x00, 0x00, 0x00, // pkt_observation_t[3]: u8 + u16
		0x01,                               // nb observations: u8
	...
	assert.Equal(t, RawRecord{
		...
		Metrics: ebpf.BpfFlowMetrics{
			EthProtocol:     0x0201,
			PktObservations: [4]ebpf.BpfPktObservationT{{Direction: 0x03, IfIndex: 0x16151413}},
			NbObservations:  1,
