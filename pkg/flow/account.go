	...
	for {
		...
		case record, ok := <-in:
			...
			if stored, ok := c.entries[record.Id]; ok {
				model.AccumulateBaseMetrics(stored, &record.Metrics)
			} else {
...
func (c *Accounter) evict(entries map[ebpf.BpfFlowId]*ebpf.BpfFlowMetrics, evictor chan<- []*model.Record, reason string) {
	...
	for key, metrics := range entries {
		records = append(records, model.NewRecord(&key, &model.BpfFlowPayload{BpfFlowMetrics: metrics}, now, monotonicNow))
	}
