...
func TestIPv4GRPCProto_ExportFlows_AgentIP(t *testing.T) {
	...
	flows <- []*model.Record{
		{RawRecord: model.RawRecord{Metrics: ebpf.BpfFlowMetrics{EthProtocol: model.IPv6Type}},
...
func TestGRPCProto_SplitLargeMessages(t *testing.T) {
	...
	for i := 0; i < 25000; i++ {
		input = append(input, &model.Record{RawRecord: model.RawRecord{Metrics: ebpf.BpfFlowMetrics{
