...
func TestIPv4GRPCProto_ExportFlows_AgentIP(t *testing.T) {
	...
	flows <- []*model.Record{
		{Metrics: &model.BpfFlowPayload{BpfFlowMetrics: &ebpf.BpfFlowMetrics{EthProtocol: model.IPv6Type}},
...
func TestGRPCProto_SplitLargeMessages(t *testing.T) {
	...
	for i := 0; i < 25000; i++ {
		input = append(input, &model.Record{Metrics: &model.BpfFlowPayload{BpfFlowMetrics: &ebpf.BpfFlowMetrics{
			EthProtocol: model.IPv6Type,
		}}, AgentIP: net.ParseIP("1111::1111"), Interfaces: []string{"12345678"}})
	}
	flows <- input
	go exporter.ExportFlows(flows)
