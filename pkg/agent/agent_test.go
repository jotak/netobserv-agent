...
func testAgent(t *testing.T, cfg *Config) *test.ExporterFake {
	...
	key1Metrics := []ebpf.BpfFlowMetrics{
		{Packets: 3, Bytes: 44, StartMonoTimeTs: now + 1000, EndMonoTimeTs: now + 1_000_000_000, NbObservations: 1, PktObservations: obsByIf3},
		{Packets: 1, Bytes: 22, StartMonoTimeTs: now, EndMonoTimeTs: now + 3000, NbObservations: 1, PktObservations: obsByIf4},
	}
	key2Metrics := []ebpf.BpfFlowMetrics{
		{Packets: 7, Bytes: 33, StartMonoTimeTs: now, EndMonoTimeTs: now + 2_000_000_000, NbObservations: 1, PktObservations: obsByIf3},
	}
