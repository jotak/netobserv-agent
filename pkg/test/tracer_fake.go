...
type TracerFake struct {
	...
	mapLookups chan map[ebpf.BpfFlowId]model.BpfFlowPayload
...
func NewTracerFake() *TracerFake {
	return &TracerFake{
		interfaces: map[ifaces.Interface]struct{}{},
		mapLookups: make(chan map[ebpf.BpfFlowId]model.BpfFlowPayload, 100),
...
func (m *TracerFake) LookupAndDeleteMap(_ *metrics.Metrics) map[ebpf.BpfFlowId]model.BpfFlowPayload {
	select {
	case r := <-m.mapLookups:
		return r
	default:
		return map[ebpf.BpfFlowId]model.BpfFlowPayload{}
	}
}
...
func (m *TracerFake) AppendLookupResults(results map[ebpf.BpfFlowId]model.BpfFlowPayload) {
